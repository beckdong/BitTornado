...
from BitTornado.Types import TypedDict, Port
...
class Tracker(object):
    ...
    def get_infopage(self):
        try:
            ...
            s.write('<h2>Peers</h2>')
            s.write('<textarea cols=128 readonly>{}</textarea>'.format(self.state['peers']))
            s.write('<h2>Cache</h2>')
            s.write('<textarea cols=128 readonly>{}</textarea>'.format(self.becache))
            s.write('</body>\n</html>\n')
            return (200, 'OK',
