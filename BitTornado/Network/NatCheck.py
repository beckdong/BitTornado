...
class NatCheck(object):
    ...
    def _read_header(self, s):
        debuglog("_read_header")
        if s == protocol_name:
            return 8, self.read_options
        return None

    def read_header(self, s):
        debuglog("read_header")
    ...
    def read_crypto_block4a(self, s):
        vc_pattern = self.encrypter.VC_pattern()
        debuglog("read_crypto_block4a({!r}, {!r})".format(s, vc_pattern))
        if not self._search_for_pattern(s, vc_pattern):
    ...
    def read_crypto_pad4(self, s):
        debuglog("read_crypto_pad4")
        # discard data
        return self.read_crypto_block4done()

    def read_crypto_block4done(self):
        debuglog("read_crypto_block4done")
    ...
    def read_encrypted_header(self, s):
        debuglog("read_encrypted_header")
        return self._read_header(s)

    ################################################

    def read_options(self, s):
        debuglog("read_options")
        return 20, self.read_download_id

    def read_download_id(self, s):
        debuglog("read_download_id")
        if s != self.downloadid:
            return None
        return 20, self.read_peer_id

    def read_peer_id(self, s):
        debuglog("read_peer_id")
        if s != self.peerid:
            return None
        self.answer(True)
        return None
    ...
    def data_came_in(self, connection, s):
        debuglog("data_came_in")
        self.read(s)

    def _write_buffer(self, s):
        debuglog("_write_buffer")
